  ...
  return (
    <Link to={`/product/${product.id}`} className="group flex flex-col w-[200px] md:w-[240px] shrink-0 h-full relative">
      ...
      <div className="flex flex-col px-1 flex-1">
        <h3 className="font-bold text-slate-800 text-[13px] md:text-sm truncate mb-4" title={product.name}>
          {product.name}
        </h3>
        ...
        <div className="flex items-end justify-between mt-auto">
          <div className="flex flex-col">
            ...
            <div className="font-extrabold text-[15px] md:text-base text-slate-900 tracking-tight">
              {hasPrice ? product.price.toLocaleString() : "가격 확인 필요"}
              {hasPrice && <span className="text-xs font-normal text-slate-500 ml-0.5">원~</span>}
            </div>
          </div>
